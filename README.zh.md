...
| **人工命令** | 在对话里输入 `/memory` 可以查看、搜索、展开或删除记忆。 |

## 面向模型（Agent）的工具

- `memory_add` — 保存一条持久事实 / 偏好 / 决策；相同标题（或相同 `id`）会更新已有记忆而不是重复新建。任何语言的标题（包括中文）都会得到稳定且互不冲突的 id。
- `memory_search` — 按关键词搜索标题、标签和正文；多词查询优先返回「全部命中」的记忆。
- `memory_list` — 列出所有记忆（可按标签过滤）。
- `memory_delete` — 按 id 删除（遗忘）一条记忆。
- `memory_summarize` — 用 LLM 把文本提炼成精简记忆并保存；显式给出 `title` 时会更新同标题的已有记忆。
