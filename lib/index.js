...
// Ids double as filenames, so they must stay within letters/digits/hyphens —
// this excludes path separators, which keeps `join(dir, `${id}.md`)` safe.
// Letters and digits are Unicode-aware so CJK titles keep distinct ids.
...
function slugify(value) {
  let slug = String(value)
    ...
    .replace(/^-+|-+$/g, "");
  if (slug.length > 60) {
    // Array.from so the cut lands between code points, not inside a surrogate pair.
    slug = Array.from(slug).slice(0, 60).join("").replace(/-+$/g, "");
  }
  // Titles that slug to nothing (symbols/emoji only) still need a stable id so
  // re-saving the same title updates instead of duplicating.
  return slug.length > 0 ? slug : `memory-${shortHash(value)}`;
}
...
/** Score one memory against a single (already lowercased) token; 0 = no match. */
function scoreMemoryToken(memory, token) {
  let score = 0;
  if (memory.title.toLowerCase().includes(token)) score += 4;
  if (memory.tags.some((tag) => tag.toLowerCase().includes(token))) score += 2;
  if (memory.content.toLowerCase().includes(token)) score += 1;
  return score;
}

/**
 * Score memories against a whitespace-separated query. Every token must match
 * somewhere (AND) — but when nothing matches all tokens, fall back to ranking
 * by total token matches so partial matches still surface. Substring matching
 * keeps CJK queries working, since they rarely contain whitespace.
 */
function searchMemories(memories, query) {
  ...
  for (const memory of memories) {
    ...
    for (const token of tokens) {
      const score = scoreMemoryToken(memory, token);
      if (score === 0) {
        allMatch = false;
        break;
      }
    ...
    if (allMatch) scored.push({ memory, score: total });
  }
  if (scored.length === 0 && tokens.length > 1) {
    for (const memory of memories) {
      let total = 0;
      for (const token of tokens) total += scoreMemoryToken(memory, token);
      if (total > 0) scored.push({ memory, score: total });
    }
  }
...
function renderMemorySummaries(memories) {
  return memories
    .map((memory) => `#${memory.id} "${memory.title}" (${memory.importance})${memory.tags.length > 0 ? ` [${memory.tags.join(", ")}]` : ""}\n  ${truncateUtf8(memory.content.replace(/\s+/g, " ").trim(), 240)}`)
    .join("\n\n");
}
...
export function apply(ctx, config = {}) {
  ...
  // Single choke point that keeps every file operation inside the memory dir.
  function memoryPath(id) {
    if (!ID_RE.test(id)) throw new Error(`invalid memory id "${id}": use letters, digits, and single hyphens`);
  ...
  async function writeMemory(memory) {
    await ensureDir();
    await atomicWriteFile(memoryPath(memory.id), serializeMemory(memory));
  }
  ...
  const addTool = defineTool({
    ...
    async execute(args, exec) {
      ...
      let id = typeof args.id === "string" && args.id.trim().length > 0 ? args.id.trim().toLowerCase() : slugify(title);
      if (!ID_RE.test(id)) throw new Error(`invalid id "${id}": use letters, digits, and single hyphens`);
  ...
  const searchTool = defineTool({
    name: "memory_search",
    description: "Search your global memories by keyword. Matches titles, tags, and body text; a multi-word query ranks memories matching ALL words first (falling back to the best partial matches). Returns the best matches with their full content.",
    parameters: {
    ...
    async execute(args, exec) {
      ...
      const scored = searchMemories(await listMemories(), query);
      const results = scored.slice(0, Math.min(limit, 50)).map(({ memory }) => ({
        id: memory.id,
        title: memory.title,
        tags: memory.tags,
        importance: memory.importance,
  ...
  const summarizeTool = defineTool({
    ...
    parameters: {
      ...
      title: { type: "string", description: "Optional title. Reusing the same title updates the existing memory instead of creating a duplicate; when omitted the model derives one and a new memory is always created." },
    ...
    output: {
      ...
      render: (_args, value) => [{
        type: "text",
        text: `${value.action === "updated" ? "Updated" : "Saved"} summarized memory #${value.id} "${value.title}"\n\n${value.summary}`
      }]
    },
    async execute(args, exec) {
      exec.signal.throwIfAborted();
      ...
      const now = new Date().toISOString();
      // An explicit title maps to a stable id (same-title updates reuse the
      // record, mirroring memory_add); a model-derived title always creates a
      // fresh entry so successive recaps don't overwrite each other.
      const id = typeof args.title === "string" && args.title.trim().length > 0 ? slugify(args.title.trim()) : newId(finalTitle);

      return await enqueueMutation(async () => {
        const existing = await readMemory(id);
        await writeMemory({ id, title: finalTitle, content: distilled.summary, tags, importance, created: existing?.created ?? now, updated: now });
        invalidateContext();
        return { id, title: finalTitle, summary: distilled.summary, action: existing === void 0 ? "created" : "updated", importance, tags, saved: true };
      });
    },
    presentCall(args) {
  ...
  if (commands !== void 0) {
    disposers.push(commands.register({
      name: "memory",
      description: "List, search, show, or delete your global memories",
      input: { hint: "[search text | show <id> | rm <id> | help]" },
      async handler(invocation) {
        ...
        if (raw.length === 0) {
          ...
          return { kind: "success", text: renderMemorySummaries(memories) };
        }

        const [verb, ...restTokens] = raw.split(/\s+/);
        ...
        if (verb === "help") {
          return { kind: "success", text: [
            ...
            "  /memory show <id>   — show one memory in full",
            "  /memory rm <id>     — delete a memory",
            "  /memory help        — this help"
          ].join("\n") };
        }

        if ((verb === "show" || verb === "rm" || verb === "delete") && arg.length > 0) {
          const id = arg.toLowerCase();
          if (!ID_RE.test(id)) {
            return { kind: "success", text: `"${arg}" is not a memory id. Run /memory to list ids, then use /memory ${verb} <id>.` };
          }
          if (verb === "show") {
            const memory = await readMemory(id);
            if (memory === void 0) return { kind: "success", text: `No memory with id "${id}". Run /memory to list ids.` };
            const meta = [`importance: ${memory.importance}`];
            if (memory.tags.length > 0) meta.push(`tags: ${memory.tags.join(", ")}`);
            if (memory.created) meta.push(`created: ${memory.created}`);
            meta.push(`updated: ${memory.updated}`);
            return { kind: "success", text: `#${memory.id} "${memory.title}"\n${meta.join("\n")}\n\n${memory.content}` };
          }
          const deleted = await deleteMemory(id);
          invalidateContext();
          return { kind: "success", text: deleted ? `Deleted memory #${id}.` : `No memory with id "${id}". Run /memory to list ids.` };
        }

        const matches = searchMemories(memories, raw).slice(0, 10).map(({ memory }) => memory);
        if (matches.length === 0) return { kind: "success", text: `No memories matched "${raw}".` };
        return { kind: "success", text: renderMemorySummaries(matches) };
      }
    }));
  }
